...
Since `before_agent_reply` fires **after** `before_prompt_build`, we can bridge the gap by storing the audio file path in `before_prompt_build` and reading it in `before_agent_reply`.

**Hook ordering:**
1. **`before_prompt_build`** – sees the media‑attachment line (contains audio file path).
2. Whisper transcription happens (JSON file created).
3. **`before_agent_reply`** – can now read the JSON transcript.

**Flow:**
1. **`before_prompt_build`** (when voice note arrives):
   - Detect audio file path in `event.prompt` (media‑attachment line).
   - Store the audio file path in a pending file: `/tmp/openclaw/multi-message-mode/pending/audio_{normalized-session-key}.txt`.
   - The file contains a single line: the audio file path.

2. **Whisper transcription** happens asynchronously; JSON file is created next to the `.ogg`.

3. **`before_agent_reply`** (immediately after):
   - Check for pending audio file for the current session.
   - If found:
     - Read the stored audio file path.
     - Derive JSON path (`audioPath.replace(/\.ogg$/, '.json')`).
     - Read the transcript (`text` property) from the JSON file.
...
**Result:**
- Voice activation works because `before_agent_reply` can read the transcript (JSON now exists).
- Voice buffering stores the transcript, not `<media:audio>` placeholder.
- Text commands (`/mmm`, `/mmc`, `/mmm‑cancel`) and text buffering continue to work.
